...
\author{K Chaitanya (25CS60R28) \and Rahul K (25CS60R16) \and\\ Sri Lalitha Rama Siddesh Devarakonda (25CS60R80)}
...
3b & High-Load Guard & Suppress drains if CPU $>75\%$ OR queue $>10$ OR booting $>0$ \\
4 & Scale-Out Suppression & Block scale-up if $\geq$2 nodes booting \\
5 & Proactive Scale-Out & If $\sigma > 0.3$ AND CPU rising, force +1 \\
6 & Queue Emergency & Force +1 VM if queue $>40$; +2 if queue $>120$ \\
\bottomrule
\end{tabular}
\end{center}
...
\end{enumerate}
\vspace{0.2cm}
Safety Filter 5 (proactive scale-out) and Filter 6 (queue emergency)
are critical for the sudden spike scenario.
\end{column}
\end{columns}
\end{frame}
...
% Slide 12: Demo Modes
\begin{frame}{Live Demo: 4 Modes}
\texttt{python demo.py} — interactive terminal demo with 4 modes:

\vspace{0.3cm}
\begin{center}
\begin{tabular}{lp{8cm}}
\toprule
\textbf{Mode} & \textbf{What It Shows} \\
\midrule
\texttt{standard} & Step-by-step: AutoCloud-Agent vs one chosen baseline \\
\texttt{shootout} & AutoCloud-Agent vs all 5 baselines with ranked table \\
\texttt{stress} & Agent vs KubernetesHPA on extreme workload (ramp, shock, choppy, trough) \\
\texttt{ablation} & 3-agent I-PPO vs Single-Agent PPO — explains decomposition value \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.3cm}
\textbf{Each step shows:} forecaster predictions $\pm\sigma$, peak queue depth,
current VM state (booting/active/draining), agent decisions, Safety Coordinator
overrides, live cost, and cumulative SLA rate.
\end{frame}
