...
\author{K Chaitanya (25CS60R28) \quad Rahul K (25CS60R16) \quad Sri Lalitha Rama Siddesh Devarakonda (25CS60R80)}
\date{April 2026}
...
\midrule
Scale-Out & Discrete(3) & 0=hold (do nothing), 1=provision 1 new VM, 2=provision 2 new VMs \\
\multicolumn{3}{l}{\footnotesize\textit{ScaleOut only ever adds capacity. Removing VMs is handled exclusively by the Consolidation agent.}} \\
...
3 & Uncertainty Suppression & If forecaster uncertainty $\sigma > 0.3$, block all
    scale-down actions \\
3b & High-Load Guard & Suppress drains if mean CPU $>$ 75\% OR queue $>$ 10 OR any
    node still booting --- prevents draining during brief false lulls \\
...
6 & Queue Emergency & Force $+$1 VM if queue $>$ 40; force $+$2 VMs if queue $>$ 120.
    Hard override when the RL agent fails to react to a severe backlog \\
\bottomrule
\end{tabular}
\end{center}
...
# Live demo (interactive wizard, 4 modes)
python demo.py
python demo.py --mode stress              # stress test, skip wizard
python demo.py --mode shootout            # RL vs all 5 baselines
python demo.py --mode ablation            # 3-agent vs single-agent
...
For a quick walkthrough with no interaction, run:
\begin{lstlisting}[language=bash]
# Stress scenario 2 (early shock), no pauses
python demo.py --mode stress --steps 60 --speed normal --no-pause

# Full shootout, fast
python demo.py --mode shootout --speed fast --no-pause

# Ablation comparison
python demo.py --mode ablation --speed fast --no-pause
\end{lstlisting}
