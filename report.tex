...
\author{\IEEEauthorblockN{Cloud Computing Term Project}
...
\begin{IEEEkeywords}
Cloud computing, autoscaling, reinforcement learning, multi-agent systems, resource management, elasticity, cost optimization
\end{IEEEkeywords}
...
\begin{enumerate}
  \item \textbf{Elasticity:} The system must scale resources up quickly when
        demand spikes to prevent SLA violations (e.g., P95 latency exceeding 500ms).
...
Recent work has applied reinforcement learning (RL) to autoscaling~\cite{zhang2023aware, mao2016resource}, demonstrating that learned policies can
...
scale-up). These methods are simple and widely deployed but purely reactive---they cannot anticipate demand changes.
...
decisions, as shown in Table~\ref{tab:hierarchy}.
...
\begin{tabular}{lcc}
\toprule
\textbf{Agent} & \textbf{Freq.} & \textbf{Rationale} \\
\midrule
Scheduling & Every step (30s) & Jobs arrive continuously \\
Consolidation & Every 2 steps (1m) & Trend observation \\
ScaleOut & Every 10 steps (5m) & VM provisioning \\
\bottomrule
\end{tabular}
\end{table}
...
A rule-based post-processing filter enforces five hard constraints
...
3 & Uncert. Suppress & If $\sigma_{t+5} > 0.3$, block drains \\
4 & Scale-Out Suppress & Block scale-up if $\geq$2 nodes booting \\
...
Node Stability & $1 - \frac{\text{std}(N)}{\text{mean}(N)}$ \\
\bottomrule
\end{tabular}
\end{table}
...
and RL (Table~\ref{tab:baselines}).
...
\midrule
Industry & KubernetesHPA & k8s HPA with 10\% dead-band \\
Industry & AWSTargetTracking & AWS asymmetric cooldowns \\
Control & MPCController & 5-step MPC with EWMA~\cite{zhang2023aware} \\
Rule & ThresholdReactive & CPU $>$80\%: add; $<$30\%: drain \\
RL ablation & SingleAgentPPO & One agent, all actions \\
...
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{SLA} & \textbf{Cost} & \textbf{CPU} & \textbf{Stab.} \\
\midrule
...
\textbf{CPU utilization:} AutoCloud-Agent achieves 55.2\%, nearly
...
The key differentiator is cost---how cheaply they maintain SLA compliance.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Temporal decomposition (matching decision freq. to task timescale)
...
\textbf{Limitations:}
Homogeneous VMs (real clouds have heterogeneous instance types);
single-cluster scope (production systems manage thousands of clusters);
simplified networking (no cross-rack or bandwidth modeling);
static reward weights (adaptive shaping could improve performance);
single-day training data (multi-day training with domain randomization
would improve generalization).

\textbf{Future Work:}
Heterogeneous VMs with bin-packing strategies;
multi-cluster federation with a hierarchical agent;
vertical scaling (resizing individual VMs);
transfer learning across different cloud traces;
spot instance awareness (preemption risk + pricing).
